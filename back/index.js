...
/**
 * GET /notificaciones
 * Obtiene las notificaciones del usuario autenticado ordenadas de la más reciente a la más antigua.
 */
app.get('/notificaciones', requireAuth, async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT n.ID, n.Titulo, n.mensaje, n.fechaLeida, n.tipoID, tn.Tipo AS tipoNombre
       FROM Notificacion n
       LEFT JOIN tipoNotificacion tn ON n.tipoID = tn.ID
       WHERE n.cuentaID = ?
       ORDER BY n.ID DESC`,
      [req.userJwt.id]
    );
    ...
    console.error('[Notificaciones] GET /notificaciones:', err);
    res.status(500).json({ error: err.message });
  }
});
 ...
 */
app.patch('/notificaciones/:id/leida', requireAuth, async (req, res) => {
  try {
    const [result] = await pool.query(
      `UPDATE Notificacion SET fechaLeida = NOW() WHERE ID = ? AND cuentaID = ?`,
      [req.params.id, req.userJwt.id]
    );
    ...
    console.error('[Notificaciones] PATCH /notificaciones/:id/leida:', err);
    res.status(500).json({ error: err.message });
  }
});
